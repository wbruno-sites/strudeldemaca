...
/**
 * Verifica se foi enviada uma requisiÁ„o POST ao servidor
 */
function isPost(){
...
/**
 * Evita SQL Injection
 * @param $var var, vari·vel a ser 'limpa'
 * @return $str string
 */
function filter( $var ){
...
/**
 * @param $string, valor que ser· limpo para URL
 */
function fazLink( $string ){
	$string = trim($string);
	
	$string = preg_replace("/[·‡‚„™]/i","a",$string);
	$string = preg_replace("/[ÈËÍ]/i","e",$string);
	$string = preg_replace("/[ÌÏÓ]/i","i",$string);
	$string = preg_replace("/[ÛÚÙı∫]/i","o",$string);
	$string = preg_replace("/[˙˘]/i","u",$string);
	$string = preg_replace("/[Á]/i","c",$string);
...
/**
 * testa se o usu·rio se identificou no login
 * @return bollean 
 */
function verifica( $level=true ){
	if ( getSession('logado') != 'ok' || !$level )
	{
		$_SESSION['logado'] = 'Usu·rio n„o identificado.';
		return false;
	}
	else
		return true;
 ...
 * verifica se È um email v·lido atravÈs da ER
 * @return boolean
 */
...
/**
 * funÁ„o tentou_invadir()
 * Grava um log chamando a class Logger, com o possÌveis erros de usu·rio
...
/**
 * Cria o diretÛrio especificado recursivamente
 * @param string $dir
 */
function cria_dir( $dir ){
	if( !is_dir( $dir ) )
		if( mkdir( $dir, 0777, true) )
			return '<strong class="system green" title="'.$dir.'">DiretÛrio criado!</strong>';
		else
			return '<strong class="system red" title="'.$dir.'">ImpossÌvel criar diretÛrio, confira as permissıes!</strong>';			
}
/**
 * Deleta o arquivo, se ele existir
 * @param $file, string contendo o caminho e o nome do arquivo ‡ ser excluido
...
/**
 * Move o arquivo temporario de upload, para a pasta com o nome j· indicado
 * @param $file, array $_FILES enviado por um formul·rio
...
/** 
 * Faz uma cÛpia da imagem, renoeando, e fazendo redimensionamento.
...
/*
 * Procura se existe tal arquivo, sem conhecer a extens„o dele
 * @param $file, arquivo com caminho completo atÈ ele
 * @return string $ext, falso ou a extens„o do arquivo, caso exista
 */
...
/**
 * Retorna a extens„o do arquivo, pegando a ultima posiÁ„o do array, explodido pelo ponto
 * @return string, .ext
 */
...
{
	$str = strtr($palavra, "·‡„‚ÈÍÌÛÙı˙¸Á¡¿√¬… Õ”‘’⁄‹« ", "aaaaeeiooouucAAAAEEIOOOUUC_");
